...
export const updateUser = AsyncHandler(async (req, res) => {
  ...
  if (email && email !== user.email) {
    ...
    if (emailExists) throw new ApiError(400, "Email already in use by another user");
    user.email = email;
  }
  ...
  if (req.file) {
    // 1. Delete existing Cloudinary image if available
    if (user.profileImgPublicId) {
      await deleteFromCloudinary(user.profileImgPublicId);
    }
    ...
    // 3. Update fields
    user.profileImg = cloudRes.secure_url;
    user.profileImgPublicId = cloudRes.public_id;
  }
  ...
  res
    ...
    .json(
      new ApiResponse(
        200,
        user,
        `User status updated to ${user.status ? "active" : "inactive"}`
      )
    );
});
